...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """ Noisy distance is estimated manhattan observation"""
        emissionModel = busters.getObservationDistribution(noisyDistance)
        """ Emission Model gives the P(noisyDistance| TrueDistance)."""
        ...
        #print "get Jail Pos",self.getJailPosition()
        #new = util.Counter()
        #for p in self.legalPositions: new[p] = 1.0
        #new.normalize()
        #print new
        #util.raiseNotDefined()
        allPossible = util.Counter()
        """ Implementing the test code to check if ghost is already captured, set all getJailPosition = 1.0"""        
        if noisyDistance == None:            
            ...
            for p in self.legalPositions:
                #print "legalPosition",p
                """Evaluate the trueDistance between the legal position p and current pacman position"""
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                #print "trueDistance",trueDistance
                #print "individual belief value", self.beliefs[p]
                """ If there exists a Probability for P(noisyDistance| TrueDistance) at trueDistance, then we update the belief by applying Bayes Rule"""
                if emissionModel[trueDistance] > 0:
                    #allPossible[p] = emissionModel[trueDistance] * new[p]
                    """ multiply probability of P(noisyDistance| TrueDistance) at trueDistance with the belief model to get exact inference"""
                    allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]
        """Normalize the updated beliefs"""
        allPossible.normalize()
        #print "allPoss normalized",allPossible
        #print "\n\n\n\n"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefProb = util.Counter()
        #print self.legalPositions
        #print self.beliefs
        """get all the possible legal positions for pacman from current state"""
        for oldPos in self.legalPositions:
            #print oldPos
            """Get the new distribution over new position for the ghost"""
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #print "NPD", newPosDist
            for newPos, prob in newPosDist.items():
                #print newPos, prob
                #beliefProb[newPos] = (prob * self.beliefs[oldPos])
                """update the belief values using Bayes Rule over the distribution and belief values"""
                beliefProb[newPos] = beliefProb[newPos] + (prob * self.beliefs[oldPos])
                #print "newBeliefValue",beliefProb[newPos]       
        #print "beliefProb",beliefProb
        self.beliefs = beliefProb
        #util.raiseNotDefined()
